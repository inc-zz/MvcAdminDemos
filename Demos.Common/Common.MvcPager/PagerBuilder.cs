...
namespace Webdiyer.WebControls.Mvc
{
    internal class PagerBuilder
    {
        ...
        private string _goToText = @"<li> <span class='input-group' style='width:20%;padding:1px 0px;margin-left:10px'>
                                                    <span class='input-group-addon'>转到第</span>
                                                    <input type='text' id='pagebox' class='form-control input-sm' value='{0}' />
                                                    <span class='input-group-addon'>页</span>
                                                    <span class='input-group-btn'><button class='btn btn-primary btn-sm' onclick='_ajax_goBtn()'>跳转</button></span>
                                                    </span> </li>";

        private string _showPageMsg = "<li><span style='background-color:transparent;border:none;color:#272727;padding-top:12px;margin-right:10px'>共{0}页 {1}条记录 当前第{2}页</span></li>";

        //html pager builder
        internal PagerBuilder(HtmlHelper htmlHelper, int totalPageCount, int pageIndex, int totalItemCount, PagerOptions pagerOptions)
        {
        ...
        public string GenerateHtml()
        {
            ...
            _showPageMsg = string.Format(_showPageMsg, _totalPageCount, _totalItemCount, _pageIndex);
            _goToText = string.Format(_goToText, _pageIndex);
            if ((_pageIndex > _totalPageCount && _totalPageCount > 0) || _pageIndex < 1)
            {
                if (_ajaxPagingEnabled)
                {
                    return string.Format("{0}<div data-ajax=\"true\" data-ajax-update=\"{1}\" data-invalidpageerrmsg=\"{2}\" data-outrangeerrmsg=\"{3}\" data-pagerid=\"Webdiyer.MvcPager\" style=\"color:red;font-weight:bold\">{3}{4}</div>{0}",
                                             _copyrightText, EscapeIdSelector(_ajaxOptions.UpdateTargetId), _pagerOptions.InvalidPageIndexErrorMessage, _pagerOptions.PageIndexOutOfRangeErrorMessage, _goToText);

                }
                return string.Format("{0}<div data-invalidpageerrmsg=\"{1}\" data-outrangeerrmsg=\"{2}\" data-pagerid=\"Webdiyer.MvcPager\" style=\"color:red;font-weight:bold\">{2}{3}</div>{0}",
                                         _copyrightText, _pagerOptions.InvalidPageIndexErrorMessage, _pagerOptions.PageIndexOutOfRangeErrorMessage, _goToText);
            }
            ...
            if ((_totalPageCount > 1 || (!_pagerOptions.AutoHide)) && !_pagerOptions.HidePagerItems)
            {
                ...
                if (_pagerOptions.IsShowPageMessage)
                    sb.Append(_showPageMsg);
