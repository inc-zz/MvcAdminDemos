namespace MongoRepository
{
        ...
        where T : IEntity<TKey>
    {
        ...
        /// <returns></returns>
        public PagedResult<T> ToPage(Expression<Func<T, bool>> predicate, int pageIndex, int pageSize, Expression<Func<T, object>> orderby)
        {
            var skip = (pageIndex - 1) * pageSize;
            var limit = pageSize;
            var recordCount = this.collection.AsQueryable<T>().Where(predicate).Count();
            return new PagedResult<T>(
                recordCount,
                (int)(recordCount + pageSize - 1) / pageSize,
                pageSize,
                pageIndex,
               this.collection.AsQueryable<T>().Where(predicate).OrderByDescending(orderby).Skip(skip).Take(limit).ToList());
        }
